...
(async function () {
   ...
   function _toStorageFormat(key, dbRecord) {
         ...
         // ── [FIX] rawTransactionDataByYear: Supabase snake_case → 앱 camelCase 변환 ──
   ...
   async function _prefetch() {
         ...
         try {
           ...
           _cache.noticesData = _toStorageFormat("noticesData", results.noticesData);
                 _cache.schedulesData = _toStorageFormat(
                           "schedulesData",
                           results.schedulesData
                         );
                 _cache.supplierAdminItems = _toStorageFormat(
                           "supplierAdminItems",
                           results.supplierAdminItems
                         );
                 _cache.gradeMacroMappings = _toStorageFormat(
                           "gradeMacroMappings",
                           results.gradeMacroMappings
                         );
                 _cache.planClipboardDataByYear = _toStorageFormat(
                           "planClipboardDataByYear",
                           results.planClipboardDataByYear
                         );
           ...
           try {
                     if (!localStorage.getItem("__supabase_migrated")) {
                                 const idbHasData = Object.keys(_cache).some(
                                               (k) =>
                                                               _cache[k] &&
                                                               (Array.isArray(_cache[k].data)
                                                                                ? _cache[k].data.length
                                                                                : Object.keys(_cache[k]).length > 0)
                                             );
                                 const sbHasData = Object.values(results).some((v) =>
   ...
   async function _prefetchTransactionsForYear(year) {
         ...
         try {
           ...
           if (!error) {
                     ...
                     // ── [FIX] Supabase에서 가져온 데이터를 앱 형식(camelCase)으로 변환하여 캐시 ──
                 ...
                 console.error(
                           `Failed to prefetch transactions for year ${year}:`,
                           err
                         );
         }
   }
      ...
      try {
           ...
           if (key === "noticesData") {
                ...
                await supabase.from("notices").delete().neq("id", "");
                     ...
                     // ── [FIX] _toDBFormat으로 변환된 snake_case + year 필드 포함 데이터 사용 ──
